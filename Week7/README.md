...
|--------------------|--------------------|--------------------|--------------------|
| Handshake Protocol | Implicit Initialization Vector (IV) used. |Initial handshake carried out in clear text, resulting in 5-7 packets exchanged. Significant overhead due to encryption and decryption. | Default server certificate encryption adopted, enabling handshake with 0-3 packets. Reduced or eliminated overhead, allowing faster connections. |
| Cipher Suites      | Limited cipher suite options. |Supports a wide range of cipher suites, including those with potential vulnerabilities. | Supports only cipher suites with no known vulnerabilities and those supporting Perfect Forward Secrecy (PFS). Shrinks the size of cipher suites for simpler, stronger encryption. |
| Key Exchange Mechanisms | Limited options. |Supports static RSA key and Diffie-Hellman key exchanges.  | Removes static RSA key exchange, retaining ephemeral Diffie-Hellman keys. Eliminates security risks associated with static keys and reduces handshake round-trip time. |
| Zero Round-Trip Time (0-RTT)| Not supported. |Not supported. | Implemented, allowing clients to send data in the first message to the server, leveraging pre-shared keys from prior sessions. Saves time and improves site performance. |
